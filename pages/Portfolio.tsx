      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-8 py-3 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${
                filter === cat 
                  ? 'bg-[#1F4E79] text-white shadow-xl scale-105' 
        ...
        {filteredProjects.length > 0 ? (
            ...
            {filteredProjects.map((project) => (
              <div key={project.id} className="group bg-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-50">
                <div className="relative h-72 overflow-hidden">
                  <img 
                    src={project.imageUrl || 'https://images.unsplash.com/photo-1484154218962-a197022b5858?auto=format&fit=crop&q=80'} 
                    alt={project.title} 
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-[#1F4E79]/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-8">
                    <span className="text-white font-bold text-sm flex items-center gap-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      Ver detalhes do projeto <ArrowRight size={18} />
                    </span>
                  </div>
                  <div className="absolute top-6 left-6">
                    <span className="bg-white/95 backdrop-blur-sm text-[#1F4E79] text-[10px] font-bold px-4 py-2 rounded-full uppercase tracking-widest shadow-lg">
                      {project.type}
                    </span>
                  </div>
                </div>
          ...
          <div className="text-center py-20">
            <p className="text-gray-400 italic">Nenhum projeto encontrado nesta categoria.</p>
          </div>
        )}
      </div>
    </div>
