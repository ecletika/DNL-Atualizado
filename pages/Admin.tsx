...
// Fixed missing Phone and Mail imports from lucide-react
import { Trash2, Edit, Plus, Star, LogOut, Check, Settings, Save, MessageSquare, FolderKanban, FileText, Eye, EyeOff, Lock, X, Clock, Upload, ImageIcon, Loader2, AlertCircle, Phone, Mail } from 'lucide-react';
...
const Admin: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<Project>>({
    title: '', description: '', type: ProjectType.RESIDENTIAL, status: ProjectStatus.IN_PROGRESS, imageUrl: '', progress: 0
  });
  ...
  const resetForm = () => {
    setFormData({ title: '', description: '', type: ProjectType.RESIDENTIAL, status: ProjectStatus.IN_PROGRESS, imageUrl: '', progress: 0 });
        ...
        <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-md border border-gray-100">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-1">
              ...
              <input 
                type="email" 
                placeholder="admin@dnl.pt" 
                className="w-full px-5 py-4 border rounded-2xl bg-gray-50 focus:bg-white focus:border-[#FFA500] outline-none transition-all font-semibold" 
                value={email} 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8">
              ...
              <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100">
                ...
                <form onSubmit={handleSubmitProject} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <button 
                        ...
                        className="text-[10px] font-bold text-[#1F4E79] hover:text-[#FFA500] uppercase tracking-widest flex items-center gap-1 disabled:opacity-50 transition-colors"
                      >
                        {isGenerating ? <Loader2 size={12} className="animate-spin" /> : <Star size={12} fill="currentColor" />}
                        Gerar com IA Gemini
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 text-xs text-gray-400">
                          Carregue a imagem de destaque do projeto.
                        </div>
                      </div>
                    </div>
                    ...
                    {!isEditing && (
                      <div className="space-y-4">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 block">Galeria (Opcional)</label>
                        <div onClick={() => galleryInputRef.current?.click()} className="w-full h-32 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-[#FFA500] hover:bg-gray-50 transition-all bg-gray-50">
                           <ImageIcon size={24} className="text-gray-300 mb-2" />
                           <span className="text-xs font-bold text-gray-400 uppercase">{galleryFiles.length > 0 ? `${galleryFiles.length} Ficheiros` : 'Fotos Adicionais'}</span>
                        </div>
                  ...
                  <div className="flex gap-4 pt-4">
                    <button type="submit" className="flex-1 bg-[#1F4E79] text-white py-4 rounded-2xl font-bold hover:bg-[#FFA500] transition-all shadow-lg">
                      {isEditing ? 'Atualizar Projeto' : 'Publicar Projeto'}
                    </button>
                    {isEditing && (
                      <button type="button" onClick={() => { setIsEditing(false); resetForm(); }} className="px-8 bg-gray-100 text-gray-500 py-4 rounded-2xl font-bold hover:bg-gray-200 transition-all">
                        Cancelar
                      </button>
                    )}
                  </div>
                ...
                <div className="divide-y divide-gray-50">
                  {projects.map(p => (
                    <div key={p.id} className="p-6 flex flex-col md:flex-row items-center gap-6 hover:bg-gray-50 transition-all">
                      <img src={p.imageUrl} alt={p.title} className="w-24 h-24 rounded-2xl object-cover shadow-sm" />
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleEditProject(p)} className="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all">
                          <Edit size={18} />
                        </button>
                        <button onClick={() => deleteProject(p.id)} className="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </div>
             ...
             <div className="space-y-6">
               ...
               {budgetRequests.length === 0 ? (
                 <div className="bg-white p-20 rounded-[3rem] text-center border-2 border-dashed border-gray-100">
                   <p className="text-gray-400 italic">Nenhum pedido recebido até ao momento.</p>
                 </div>
               ) : (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   {budgetRequests.map(r => (
                     <div key={r.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 relative group">
                       <div className="flex justify-between items-start mb-6">
                         <div className={`px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${r.status === 'pendente' ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500'}`}>
                           {r.status}
                         </div>
                         <button onClick={() => deleteBudgetRequest(r.id)} className="text-gray-300 hover:text-red-500 transition-colors">
                           <Trash2 size={18} />
                         </button>
                       </div>
                       ...
                       <div className="space-y-3 mb-8">
                         <div className="flex items-center gap-3 text-sm text-gray-500">
                           <Phone size={16} className="text-gray-300" /> {r.phone}
                         </div>
                         <div className="flex items-center gap-3 text-sm text-gray-500">
                           <Mail size={16} className="text-gray-300" /> {r.email}
                         </div>
                         <div className="flex items-center gap-3 text-sm text-gray-500">
                           <Clock size={16} className="text-gray-300" /> {new Date(r.created_at).toLocaleDateString('pt-PT')}
                         </div>
                       </div>
                       ...
                       <select 
                         value={r.status} 
                         onChange={(e) => updateBudgetStatus(r.id, e.target.value as any)}
                         className="w-full p-4 rounded-xl border border-gray-100 font-bold text-xs uppercase tracking-widest outline-none bg-white cursor-pointer hover:border-[#FFA500] transition-all"
                       >
          ...
          {activeTab === 'reviews' && (
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
               <div className="p-8 border-b border-gray-50 bg-gray-50/50">
                  <h3 className="text-xl font-['Oswald'] font-bold text-[#1F4E79] uppercase">Moderação de Avaliações</h3>
               </div>
               <div className="divide-y divide-gray-50">
                 {reviews.length === 0 ? (
                   <div className="p-20 text-center text-gray-400 italic">Sem avaliações pendentes ou publicadas.</div>
                 ) : (
                   reviews.map(r => (
                     <div key={r.id} className="p-8 flex flex-col md:flex-row gap-8">
                       <div className="w-16 h-16 bg-[#1F4E79] rounded-full flex items-center justify-center text-white text-2xl font-bold shrink-0">
                         {r.clientName.charAt(0)}
                       </div>
                       <div className="flex-1">
                         <div className="flex justify-between items-start mb-2">
                           <div>
                             <h4 className="font-bold text-[#1F4E79] text-lg">{r.clientName}</h4>
                             <div className="flex text-[#FFA500] my-1">
                         ...
                         <div className="flex gap-3">
                           <button 
                             ...
                             className={`px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-all ${r.approved ? 'bg-green-50 text-green-600' : 'bg-[#FFA500] text-white'}`}
                           >
                             {r.approved ? <><Check size={14} /> Publicado</> : 'Aprovar'}
                           </button>
                           <button onClick={() => handleDeleteReview(r.id)} className="px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-widest bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-all">
                             Excluir
                           </button>
                         </div>
                       </div>
          ...
          {activeTab === 'settings' && (
            <div className="bg-white p-8 md:p-12 rounded-[3rem] shadow-sm border border-gray-100">
              ...
              <div className="max-w-2xl space-y-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Email de Notificação</label>
                  <input type="email" value={notificationEmail} onChange={e => setNotificationEmail(e.target.value)} className="w-full px-5 py-4 border-2 border-gray-50 rounded-2xl bg-gray-50 focus:bg-white focus:border-[#FFA500] outline-none transition-all font-semibold" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Link do Logótipo Personalizado</label>
                  <input type="text" value={logoUrl} onChange={e => setLogoUrl(e.target.value)} className="w-full px-5 py-4 border-2 border-gray-50 rounded-2xl bg-gray-50 focus:bg-white focus:border-[#FFA500] outline-none transition-all font-semibold" placeholder="https://..." />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowApiKey(!showApiKey)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#1F4E79]">
                ...
                <div className="flex flex-col gap-4">
                  {saveStatus === 'success' && (
                    <div className="flex items-center gap-2 text-green-600 bg-green-50 p-4 rounded-xl font-bold animate-fade-in">
                      <Check size={20} /> Definições guardadas com sucesso!
                    </div>
                  )}
                  {saveStatus === 'error' && (
                    <div className="flex items-center gap-2 text-red-600 bg-red-50 p-4 rounded-xl font-bold animate-fade-in">
                      <AlertCircle size={20} /> Ocorreu um erro ao guardar. Tente novamente.
                    </div>
                  )}
                  ...
                  <button 
                    ...
                    className={`bg-[#1F4E79] text-white px-10 py-4 rounded-2xl font-bold hover:bg-[#FFA500] transition-all shadow-lg flex items-center justify-center gap-3 disabled:opacity-50`}
                  >
